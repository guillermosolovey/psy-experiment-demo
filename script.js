...
beginPracticeBtn?.addEventListener("click", () => {
  const inputCode = sessionCodeInput.value.trim();
  state.sessionCode = inputCode || localStorage.getItem("lastSessionCode") || "";
  if (state.sessionCode) {
    localStorage.setItem("lastSessionCode", state.sessionCode);
  }
...
const initializeApp = () => {
  if (isDashboard()) {
    ...
    const paramCode = getSessionParam();
    dashboardSessionInput.value = paramCode || storedCode;
    if (paramCode) {
      localStorage.setItem("dashboardSessionCode", paramCode);
    }
  ...
  const paramSession = getSessionParam();
  if (paramSession) {
    sessionCodeInput.value = paramSession;
    localStorage.setItem("lastSessionCode", paramSession);
  } else {
    sessionCodeInput.value = localStorage.getItem("lastSessionCode") || "";
  }

  const hasState = loadState();
  if (hasState && state.phase !== "landing") {
    conditionInstruction.textContent = instructions[state.condition] || "";
    sessionCodeInput.value = state.sessionCode || sessionCodeInput.value;
